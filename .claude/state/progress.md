...
## Current feature
**LV1 · live-voice host — ✅ PO-ACCEPTED (conditionally, 2026-06-03).** Native `gemini-3.1-flash-live-preview`
...
at demo (the §13 voice call — does it *sound* right). **RELEASE remaining** = commit+tag + ECS Express Mode
staging deploy (infra-gated). **NEXT MILESTONE: M4** (two-level ranking + "while you were gone" digest — the one
...
- **Layer:** backend
- **Phase:** off (**✅ BROWSER-DIRECT PIVOT — CODE-COMPLETE + RE-QA PASSED** — topology (a), ADR 0007 Amendment C). Re-qa drove the WIRED FE live: `POST /live/session → 200` → WS **DIRECT to `generativelanguage.googleapis.com/…Constrained?access_token=`** (NO relay; only-WS-host = Google) → sent **setup→clientContent** → **34 PCM frames** → host speaking ≈8s ≈ audio → idle (drain-coupled, LV2-D1 live); cut/no-spoiler/silence-budget/secrets-from-env all PASS; **no new defects**. `pnpm verify`=0 (backend **16** / frontend **37**). **NO ECS — App Runner keeps only the HTTP mint** (saved ~$25-30/mo; reversed Amendment A2). REMAINING: **PO RE-accept** (transport changed) → dev-ops commit/tag (supersede `lv1`/`78b1d9f` + commit the `chore: teamentic 0.5.0` delta) + deploy (App Runner mint + S3/CloudFront FE `VITE_LIVE_VOICE=1`, NO relay/ECS) + verify → **M4**. _Follow-up: architect reconcile ADR 0007/design-notes/backlog to topology (a) — relay refs now historical._) _NB 0.5.0: neutral = `off`._
