...
|------|-----------|-----|--------|------------------|-------|
| 2026-06-03 | LV1 — live-voice host (Gemini Live audio over WSS relay) | `lv1` | _the `lv1`-tagged commit_ (`git rev-parse lv1`) | ⏳ **NOT deployed — ECS-pending** (App Runner cannot host the WS relay; staging deploy gated on the ECS Express Mode standup, phase 2) | **Status: committed + tagged LOCALLY (no push, no deploy yet).** `feat(lv1): live-voice host — Gemini Live streamed audio over a backend WSS relay`. PO-accepted (conditionally — navigator's audible/voice-identity confirmation outstanding) 2026-06-03; `pnpm verify`=0 at the tagged commit (**backend 21, frontend 37**, e2e 1 skipped, typecheck+lint clean). **Annotated tag `lv1`** on this commit. **Secrets gate CLEAN:** `backend/.env` (real `GEMINI_API_KEY`) gitignored + unstaged; no `frontend/.env.local`; only `backend/.env.example` tracked; staged-diff secret grep (`AIza`/`GEMINI_API_KEY=`/`access_token=`/`auth_tokens/`) empty on both commits; throwaway probes `backend/scripts/` gitignored (read the key from env, never committed). **LV1 = two commits:** the prior `9f7f07d` landed the backend mint module (`/live/session`, `live-token-client`, `live-setup`, schema) + its tests + ADR 0007/0005; **this feat commit completes LV1** — the **WSS relay** (`backend/src/modules/live/live-relay.ts` + `live-relay.routes.ts`, `@fastify/websocket`@8 + `ws`@8 + `@google/genai`@2.7 + `@types/ws`), `app.ts` wiring, the FE narrator/relay-client/audio-sink (`frontend/src/lib/{live-narrator,live-relay-client,audio-sink}.ts`), `App.tsx` (`liveVoice` behind `VITE_LIVE_VOICE`), `character.tsx` (`speakingMs` safety cap), and the live-* tests. **Topology b** (browser → backend WSS relay → Google Live WSS): the relay attaches the ephemeral token upstream + sends `setup` once; FE sends only `clientContent`. Live-validated against the real Gemini Live account (`live-probe`/`relay-probe`, throwaway): mint → WSS → `setup` → speak → PCM `audio/pcm;rate=24000` → `turnComplete`; caught + fixed 3 ADR-0007 wire corrections (auth `?access_token=` not `Authorization: Token`; `…Constrained` method; mint `name`→`token` + synth `expiresAt`) + a `@fastify/websocket` v8 registration bug. Closed defects LV1-D1 (binary frames), LV2-D1 (`speakingMs` cap), LV2-D2 (relay close-propagation). **Deploy = phase 2** (real-AWS ECS Express Mode relay) — see Cadence / Decisions. **Not pushed** (a push CI-deploys to App Runner, which cannot host the relay → would ship a broken relay; push waits for the ECS infra). |
