...
describe("App digest", () => {
  ...
  it("voices the 'while you were gone' digest first on Start watching, exactly once, and never before the gesture", async () => {
    ...
    // A recording VoiceNarrator: each speak() pushes its text in call order. A
    // never-resolving promise is fine — this test pins the ORDER and COUNT of
    // what is voiced, not the drain-coupled revert.
    const spoken: string[] = [];
    const voice = {
      speak: (t: string) => {
        spoken.push(t);
    ...
    // 3. Event follows, digest once: advancing past ts:1 surfaces the first live
    //    event, voiced AFTER the digest. The digest is not re-voiced per event —
    //    it appears exactly once across everything spoken (cost-gating).
    await act(async () => {
